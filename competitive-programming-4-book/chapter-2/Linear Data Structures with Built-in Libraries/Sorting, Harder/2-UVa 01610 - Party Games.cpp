...
int main() {
    ...
    while (true) {
        ...
        for (int i = 0; i < min(arr[idx].length(), arr[idx + 1].length()); i++) {
            ...
            if (arr[idx][i] + 1 != arr[idx + 1][i] && i == arr[idx].size() - 1) {
                res += arr[idx][i];
                lastIdx++;
                break;
            }

            if (arr[idx][i] + 1 != arr[idx + 1][i]) {
                res += arr[idx][i] + 1;
                lastIdx++;
                break;
            }
        ...
        while (lastIdx < arr[idx].size() - 1 && res < arr[idx]) {
            if (arr[idx][lastIdx] == 'Z') {
                res += arr[idx][lastIdx];
            } else {
                res += arr[idx][lastIdx] + 1;
            }
            lastIdx++;
        }
