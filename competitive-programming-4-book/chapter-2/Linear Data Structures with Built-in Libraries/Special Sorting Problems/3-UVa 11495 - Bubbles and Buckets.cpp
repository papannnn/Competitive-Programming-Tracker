...
using namespace std;
// https://vjudge.net/problem/UVA-11495
// O(t * n log n)
void merge(vector<int> &arr, int l, int mid, int r, int &inversion) {
    int ptrL = l;
    int ptrR = mid + 1;
    vector<int> temp;
...
void mergeSort(vector<int> &arr, int l, int r, int &inversion) {
    if (l < r) {
        int mid = (l + r) / 2;
...
int main () {
    ...
    while (cin >> n) {
        ...
        cout << (inversion % 2 ? "Marcelo" : "Carlos") << endl;
    }
}